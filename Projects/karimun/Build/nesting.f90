      MODULE nesting_mod
!
!svn $Id: nesting.F 1070 2021-06-08 21:00:47Z arango $
!=======================================================================
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 36, 189, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 36, 371, MyFile)
!
      RETURN
...
!
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Nonlinear/nesting.F"//", bry_fluxes"
!
!-----------------------------------------------------------------------
...
!
      ...
      IF (FoundError(exit_flag, NoError, 627, MyFile)) RETURN
...
!
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Nonlinear/nesting.F"//", mask_hweights"
!
!=======================================================================
...
!
          CALL mp_assemble (dg, model, Lpoints, spv, LW)
          IF (FoundError(exit_flag, NoError, 1724, MyFile)) RETURN
!
!  Load exchanged weights.
!
...
!
          CALL mp_assemble (dg, model, Lpoints, spv, LW)
          IF (FoundError(exit_flag, NoError, 1954, MyFile)) RETURN
!
!  Load exchanged weights.
!
...
!
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Nonlinear/nesting.F"//", get_metrics"
!
!-----------------------------------------------------------------------
...
!
        ...
        IF (FoundError(exit_flag, NoError, 2344, MyFile)) RETURN
        CALL mp_assemble (dg, model, NpointsV, spv, REFINED(cr) % om_v)
        IF (FoundError(exit_flag, NoError, 2347, MyFile)) RETURN
      END DO
!
      RETURN
...
!
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Nonlinear/nesting.F"//", fine2coarse"
!
!-----------------------------------------------------------------------
...
!
     ...
     &                          COUPLING(dg)%Zt_avg1,                   &
     &                          COUPLING(rg)%Zt_avg1)
            IF (FoundError(exit_flag, NoError, 3610, MyFile)) RETURN
!
...
!
     ...
     &                          OCEAN(rg)%ubar(:,:,1),                  &
     &                          OCEAN(rg)%ubar(:,:,2))
            IF (FoundError(exit_flag, NoError, 3637, MyFile)) RETURN
!
            CALL fine2coarse2d (rg, dg, model, tile,                    &
     &                          v2dvar, Vname(1,idVbar),                &
     &                          AreaAvg, RefineScale(dg),               &
     ...
     &                          OCEAN(rg)%vbar(:,:,1),                  &
     &                          OCEAN(rg)%vbar(:,:,2))
            IF (FoundError(exit_flag, NoError, 3661, MyFile)) RETURN
!
!-----------------------------------------------------------------------
...
!
     ...
     &                          OCEAN(dg)%u(:,:,:,Dindex3d),            &
     &                          OCEAN(rg)%u(:,:,:,Rindex3d))
            IF (FoundError(exit_flag, NoError, 3714, MyFile)) RETURN
!
            CALL fine2coarse3d (rg, dg, model, tile,                    &
     &                          v3dvar, Vname(1,idVvel),                &
     &                          AreaAvg, RefineScale(dg),               &
...
!
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Nonlinear/nesting.F"//", fine2coarse2d"
!
!-----------------------------------------------------------------------
...
!
     ...
     &                       LBi,  UBi,  LBj,  UBj,                     &
     &                       Adx, dxF)
        IF (FoundError(exit_flag, NoError, 4059, MyFile)) RETURN
        CALL mp_aggregate2d (dg, model, gtype,                          &
     &                       LBiF, UBiF, LBjF, UBjF,                    &
     &                       LBi,  UBi,  LBj,  UBj,                     &
     &                       Ady, dyF)
        IF (FoundError(exit_flag, NoError, 4065, MyFile)) RETURN
      END IF
!
      CALL mp_aggregate2d (dg, model, gtype,                            &
     &                     LBiF, UBiF, LBjF, UBjF,                      &
     &                     LBi,  UBi,  LBj,  UBj,                       &
     &                     Amsk, Fmsk)
      IF (FoundError(exit_flag, NoError, 4073, MyFile)) RETURN
!
...
!
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Nonlinear/nesting.F"//", fine2coarse3d"
!
!-----------------------------------------------------------------------
!  Set lower and upper tile bounds and staggered variables bounds for
!  this horizontal domain partition.  Notice that if tile=-1, it will
...
!
     ...
     &                     LBkF, UBkF,                                  &
     &                     A, F)
      IF (FoundError(exit_flag, NoError, 4533, MyFile)) RETURN
!
      IF (AreaAvg) THEN
        CALL mp_aggregate2d (dg, model, gtype,                          &
     &                       LBiF, UBiF, LBjF, UBjF,                    &
     &                       LBi,  UBi,  LBj,  UBj,                     &
     &                       Adx, dxF)
        IF (FoundError(exit_flag, NoError, 4540, MyFile)) RETURN
        CALL mp_aggregate2d (dg, model, gtype,                          &
     &                       LBiF, UBiF, LBjF, UBjF,                    &
     &                       LBi,  UBi,  LBj,  UBj,                     &
     &                       Ady, dyF)
        IF (FoundError(exit_flag, NoError, 4546, MyFile)) RETURN
      END IF
!
      CALL mp_aggregate2d (dg, model, gtype,                            &
     &                     LBiF, UBiF, LBjF, UBjF,                      &
     &                     LBi,  UBi,  LBj,  UBj,                       &
     &                     Amsk, Fmsk)
      IF (FoundError(exit_flag, NoError, 4554, MyFile)) RETURN
!
!-----------------------------------------------------------------------
...
!
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Nonlinear/nesting.F"//", get_persisted2d"
!
!-----------------------------------------------------------------------
!  Initialize.
!-----------------------------------------------------------------------
...
!
      CALL mp_assemble (dg, model, Npts, Aspv, Ac)
      IF (FoundError(exit_flag, NoError, 5316, MyFile)) RETURN
!
      RETURN
...
!
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Nonlinear/nesting.F"//", put_refined2d"
!
!-----------------------------------------------------------------------
!  Set lower and upper tile bounds and staggered variables bounds for
!  this horizontal domain partition.  Notice that if tile=-1, it will
...
!
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Nonlinear/nesting.F"//", put_refined3d"
!
!-----------------------------------------------------------------------
!  Set lower and upper tile bounds and staggered variables bounds for
!  this horizontal domain partition.  Notice that if tile=-1, it will
...
!
     ...
     &                        INT(RollingTime(tnew,cr))
          END IF
          exit_flag=8
          IF (FoundError(exit_flag, NoError, 6219, MyFile)) RETURN
        END IF
      END IF
!
!  Tracer-type variables.
...
!
      character (len=*), parameter :: MyFile =                          &
     &  "ROMS/Nonlinear/nesting.F"//", z_weights"
!
!=======================================================================
...
!
            ...
            IF (FoundError(exit_flag, NoError, 6566, MyFile)) RETURN
!
...
!
          ...
          IF (FoundError(exit_flag, NoError, 6626, MyFile)) RETURN
          CALL mp_assemble (rg, model, Nwpts, spv, Rcontact(cr)%Vweight)
          IF (FoundError(exit_flag, NoError, 6629, MyFile)) RETURN
!
!  Deallocate local work arrays.
!
...
!
          ...
          IF (FoundError(exit_flag, NoError, 6796, MyFile)) RETURN
          CALL mp_assemble (rg, model, Nwpts, spv, Ucontact(cr)%Vweight)
          IF (FoundError(exit_flag, NoError, 6799, MyFile)) RETURN
!
!  Deallocate local work arrays.
!
          IF (allocated(Zd)) THEN
...
!
            CALL mp_assemble (dg, model, Nzpts, spv, Zd)
            IF (FoundError(exit_flag, NoError, 6901, MyFile)) RETURN
!
!  Determine donor grid vertical indices (Kdg) and weights (Vweight)
!  needed for the interpolation of data at the receiver grid contact
!  points.
...
!
          ...
          IF (FoundError(exit_flag, NoError, 6962, MyFile)) RETURN
          CALL mp_assemble (rg, model, Nwpts, spv, Vcontact(cr)%Vweight)
          IF (FoundError(exit_flag, NoError, 6965, MyFile)) RETURN
!
!  Deallocate local work arrays.
!
          IF (allocated(Zd)) THEN
