...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 72, 145, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 72, 173, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 424, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 462, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 519, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 558, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 617, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 658, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 717, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 759, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 817, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 860, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 916, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 954, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1011, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1050, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1108, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1149, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1206, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1244, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1301, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1340, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1398, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1439, MyFile)
!
      RETURN
...
!
      IF (Lwclock) THEN
        CALL wclock_on (ng, model, 64, 1496, MyFile)
      END IF
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
...
!
      IF (Lwclock) THEN
        CALL wclock_off (ng, model, 64, 1536, MyFile)
      END IF
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1593, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1633, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1731, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1788, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 1830, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 64, 1888, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 64, 2016, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 68, 2096, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 68, 2198, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 2272, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 2433, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 2520, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 2696, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 2783, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 2960, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 3043, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 3204, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 70, 3290, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 70, 3467, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 69, 3549, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 69, 3693, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 69, 3767, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 69, 3911, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 4015, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 4246, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 4353, MyFile)
!
!-----------------------------------------------------------------------
!  Set horizontal starting and ending indices for parallel domain
!  partitions in the XI- and ETA-directions.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 4605, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 4672, MyFile)
!
!-----------------------------------------------------------------------
!  Collect data from all nodes.
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 4754, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 4837, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 4965, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 5057, MyFile)
!
!-----------------------------------------------------------------------
!  Read requested NetCDF file and scatter it to all nodes.
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 5195, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 5285, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 5419, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 66, 5511, MyFile)
!
!-----------------------------------------------------------------------
!  Collect and write data into requested NetCDF file.
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 66, 5655, MyFile)
!
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 65, 6158, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 65, 6287, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 65, 6360, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 65, 6504, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 65, 6575, MyFile)
!
!-----------------------------------------------------------------------
!  Set distributed-memory communicator handle (context ID).
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 65, 6643, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 6736, MyFile)
!
!-----------------------------------------------------------------------
!  Set horizontal starting and ending indices for parallel domain
!  partitions in the XI- and ETA-directions.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 6918, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 7013, MyFile)
!
!-----------------------------------------------------------------------
!  Set horizontal starting and ending indices for parallel domain
!  partitions in the XI- and ETA-directions.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 7208, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 67, 7280, MyFile)
!
!-----------------------------------------------------------------------
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 67, 7377, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 71, 7581, MyFile)
!
!-----------------------------------------------------------------------
!  Set horizontal starting and ending indices for parallel domain
!  partitions in the XI- and ETA-directions.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 71, 7711, MyFile)
!
      RETURN
...
!-----------------------------------------------------------------------
!
      CALL wclock_on (ng, model, 71, 7795, MyFile)
!
!-----------------------------------------------------------------------
!  Set horizontal starting and ending indices for parallel domain
!  partitions in the XI- and ETA-directions.
...
!-----------------------------------------------------------------------
!
      CALL wclock_off (ng, model, 71, 7931, MyFile)
!
      RETURN
