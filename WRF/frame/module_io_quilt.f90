...
mess)
        ...
        IF ( obufsize .GT. 0 ) THEN
          ...
          CALL collect_on_comm_debug("module_io_quilt_old.F",728, mpi_comm_io_groups(1),        &
                                onebyte,                      &
                                dummy, 0,                     &
                                obuf, obufsize )
        ...
        DO WHILE ( retval ) 
          ...
          CALL collect_on_comm_debug2("module_io_quilt_old.F",958,Trim(VarName),        &
...
mess)
        ...
        IF ( obufsize .GT. 0 ) THEN
          ...
          CALL collect_on_comm_debug("module_io_quilt_old.F",1730, mpi_comm_io_groups(1),        &
                                onebyte,                      &
                                dummy, 0,                     &
                                obuf, obufsize )
...
SUBROUTINE quilting_disabled( reslt )
  ...
  reslt = disable_quilt
write(0,*)"module_io_quilt_old.F",2950,disable_quilt
  RETURN
...
SUBROUTINE wrf_quilt_open_for_write_begin( FileName , gridid, Comm_compute, Comm_io, SysDepInfo, &
  ...
  CALL collect_on_comm_debug("module_io_quilt_old.F",3066, comm_io_group,            &
                        onebyte,                       &
                        hdrbuf, hdrbufsize , &
                        dummy, 0 )
...
SUBROUTINE wrf_quilt_open_for_write_commit( DataHandle , Status )
  ...
  CALL collect_on_comm_debug("module_io_quilt_old.F",3142, comm_io_group,            &
                        onebyte,                       &
                        hdrbuf, hdrbufsize , &
                        dummy, 0 )
...
SUBROUTINE wrf_quilt_iosync ( DataHandle, Status )
  ...
  IF ( associated ( int_local_output_buffer ) ) THEN
    ...
    CALL collect_on_comm_debug("module_io_quilt_old.F",3309, comm_io_group,            &
                          onebyte,                       &
...
SUBROUTINE wrf_quilt_set_time ( DataHandle, Data,  Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      CALL collect_on_comm_debug("module_io_quilt_old.F",3581, comm_io_group,            &
                            onebyte,                       &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_dom_ti_real ( DataHandle,Element,   Data, Count,  Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      CALL collect_on_comm_debug("module_io_quilt_old.F",3699, comm_io_group,            &
                            onebyte,                       &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_dom_ti_integer ( DataHandle,Element,   Data, Count,  Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      CALL collect_on_comm_debug("module_io_quilt_old.F",3859, comm_io_group,            &
                            onebyte,                       &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
SUBROUTINE wrf_quilt_put_dom_ti_char ( DataHandle, Element,  Data,  Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      CALL collect_on_comm_debug("module_io_quilt_old.F",4030, comm_io_group,            &
                            onebyte,                       &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
...
SUBROUTINE wrf_quilt_put_var_ti_char ( DataHandle,Element,  Varname, Data,  Status )
  ...
  IF ( DataHandle .GE. 1 .AND. DataHandle .LE. int_num_handles ) THEN
    IF ( int_handle_in_use( DataHandle ) ) THEN
      ...
      CALL collect_on_comm_debug("module_io_quilt_old.F",4562, comm_io_group,            &
                            onebyte,                       &
                            hdrbuf, hdrbufsize , &
                            dummy, 0 )
    ENDIF
...
"frame/module_io_quilt.F: wrf_quilt_write_field: DataHandle not opened" )
  ...
  IF ( .NOT. okay_to_write( DataHandle ) ) THEN
      ...
      CALL collect_on_comm_debug("module_io_quilt_old.F",4984, comm_io_group,                   &
                            onebyte,                          &
                            hdrbuf, hdrbufsize ,                 &
                            dummy, 0 )
