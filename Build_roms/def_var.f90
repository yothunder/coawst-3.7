      MODULE def_var_mod
!
!svn $Id: def_var.F 1054 2021-03-06 19:47:12Z arango $
!================================================== Hernan G. Arango ===
...
!
     ...
     &                            Vdim(1:nVdim), Vid)
            END IF
            IF (FoundError(status, nf90_noerr, 143, MyFile)) THEN
...
!
     ...
     &                          'grid_topology')
            IF (FoundError(status, nf90_noerr, 182, MyFile)) THEN
...
!
     ...
     &                          TRIM(text))
            IF (FoundError(status, nf90_noerr, 225, MyFile)) THEN
...
!
     ...
     &                            'lon_u lat_u')
              IF (FoundError(status, nf90_noerr, 267, MyFile)) THEN
     ...
     &                            'lon_v lat_v')
              IF (FoundError(status, nf90_noerr, 277, MyFile)) THEN
     ...
     &                                        TRIM(ncname)
            ...
            ELSE
              status=nf90_put_att(ncid, Vid, 'node_coordinates',        &
     &                            'x_psi y_psi')
              IF (FoundError(status, nf90_noerr, 287, MyFile)) THEN
                IF (Master) WRITE (stdout,30) 'node_coordinates',       &
     &                                        TRIM(Vinfo(1)),           &
     &                                        TRIM(ncname)
                exit_flag=3
     ...
     &                                        TRIM(ncname)
              ...
              status=nf90_put_att(ncid, Vid, 'edge1_coordinates',       &
     &                            'x_u y_u')
              IF (FoundError(status, nf90_noerr, 307, MyFile)) THEN
                IF (Master) WRITE (stdout,30) 'edge1_coordinates',      &
     &                                        TRIM(Vinfo(1)),           &
     &                                        TRIM(ncname)
                exit_flag=3
              ...
              status=nf90_put_att(ncid, Vid, 'edge2_coordinates',       &
     &                            'x_v y_v')
              IF (FoundError(status, nf90_noerr, 317, MyFile)) THEN
                IF (Master) WRITE (stdout,30) 'edge2_coordinates',      &
     &                                        TRIM(Vinfo(1)),           &
     &                                        TRIM(ncname)
                exit_flag=3
...
!
     ...
     &                          Vinfo(19)(1:latt))
            IF (FoundError(status, nf90_noerr, 365, MyFile)) THEN
...
!
     ...
     &                            (/0.0_r8, 1.0_r8/))
            ...
            IF (status.eq.nf90_noerr) THEN
              ...
              status=nf90_put_att(ncid, Vid, 'flag_meanings',           &
     &                            text(1:latt))
              IF (FoundError(status, nf90_noerr, 522, MyFile)) THEN
...
!
     ...
     &                          Vinfo(25)(1:latt))
            IF (FoundError(status, nf90_noerr, 580, MyFile)) THEN
...
!
     ...
     &                          Vinfo(16)(1:latt))
            IF (FoundError(status, nf90_noerr, 631, MyFile)) THEN
...
!
     ...
     &                            Aval(4))
            END IF
            IF (FoundError(status, nf90_noerr, 658, MyFile)) THEN
...
!
     ...
     &                            Aval(6))
            END IF
            IF (FoundError(status, nf90_noerr, 714, MyFile)) THEN
...
!
     ...
     &                          text(1:latt))
            IF (FoundError(status, nf90_noerr, 890, MyFile)) THEN
...
!
     ...
     &                          Vinfo(23)(1:latt))
            IF (FoundError(status, nf90_noerr, 908, MyFile)) THEN
...
!
     ...
     &                          Vinfo(14)(1:latt))
            IF (FoundError(status, nf90_noerr, 925, MyFile)) THEN
